...
if (!text)
    return api.sendMessage("Add text lmao", event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://api.zahwazein.xyz/textpro/pornhub?apikey=zenzkey_a1780450fcc8&text=${text}&text2=${text2}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}
